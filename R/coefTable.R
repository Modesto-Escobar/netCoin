## coefTable() -- propuesta para netCoin
## Tabla comparativa de coeficientes de uno o varios modelos glmCoin.
...
coefTable <- function(G,
  ...
  columns <- match.arg(columns, c("b", "sig", "se", "ci", "p"), several.ok = TRUE)
  ...
  ok <- vapply(G, function(x) is.list(x) && all(c("links", "equations") %in% names(x)), NA)
  if (!all(ok)) stop("element(s) ", paste(which(!ok), collapse = ", "),
                     " of 'G' are not glmCoin objects (missing $links or $equations).")

  ## ---- 2. Indice de columnas: (objeto, ecuacion, target) --------------------
  idx <- do.call(rbind, lapply(seq_along(G), function(g) {
    li <- G[[g]]$links
    mo <- if (!is.null(li$Model)) li$Model else G[[g]]$equations$model[1]
    do.call(rbind, lapply(unique(mo), function(m)
      data.frame(g = g, model = m, target = unique(li$Target[mo == m]),
                 stringsAsFactors = FALSE)))
  }))
  nb <- nrow(idx)

  ## links$Model conserva el sufijo de familia (", binomial"); equations$model no.
  ## Se intenta coincidencia exacta, luego sin familia, y por ultimo por la VD.
  normMod <- function(x) trimws(gsub("`", "", sub(",[^,]*$", "", x)))
  idx$eq <- vapply(seq_len(nb), function(j) {
    eqs <- G[[idx$g[j]]]$equations
    i <- match(idx$model[j], eqs$model)
    if (is.na(i)) i <- match(normMod(idx$model[j]), normMod(eqs$model))
    if (is.na(i)) i <- match(sub("~.*$", "", normMod(idx$model[j])),
                             sub("~.*$", "", normMod(eqs$model)))
    if (is.na(i))
      warning("no equation in $equations matches '", idx$model[j],
              "'; its fit statistics are omitted.", call. = FALSE)
    i
  }, 1L)
  eqval <- function(f) vapply(seq_len(nb), function(j) {
    eqs <- G[[idx$g[j]]]$equations
    if (is.null(eqs[[f]]) || is.na(idx$eq[j])) NA_real_
    else as.numeric(eqs[[f]][idx$eq[j]])
  }, 0)
  ...
  srcOf <- lapply(seq_len(nb), function(j) {
    li <- G[[idx$g[j]]]$links
    mo <- if (!is.null(li$Model)) li$Model else idx$model[j]
    li$Source[mo == idx$model[j] & li$Target == idx$target[j]]
  })
  ...
  sub_ <- c(b = "b", sig = "sig.", se = "(SE)", ci = "95% CI", p = "p")[columns]
  k <- length(sub_)

  cell <- function(j) {
    li <- G[[idx$g[j]]]$links
    mo <- if (!is.null(li$Model)) li$Model else idx$model[j]
    d  <- li[mo == idx$model[j] & li$Target == idx$target[j], ]
    i  <- match(vars, d$Source)
    out <- cbind(
      b        = num(fb, d$estimate[i]),
      sig.     = stars(d$p.value[i]),
      `(SE)`   = num(paste0("(", fs, ")"), d$std.error[i]),
      `95% CI` = ifelse(is.na(i), "",
                        sprintf(paste0("[", fs, ", ", fs, "]"),
                                d$conf.low[i], d$conf.high[i])),
      p        = num("%.3f", d$p.value[i]))
    out[, sub_, drop = FALSE]
  }
  ...
  ## ---- 7. Cuerpo ------------------------------------------------------------
  M <- cbind(Variable = vlab, do.call(cbind, lapply(seq_len(nb), cell)))
  ...
  firstEq <- !duplicated(paste(idx$g, idx$model))
  for (f in footer) {
